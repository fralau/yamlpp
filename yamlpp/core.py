...
from .util import yaml, load_yaml, dequote, get_line_number
...
def get(block: dict|list, index:str|int, err_msg:str="Item not found"):
    """
    Get a value from a node, and raise an error if not found.
    The block must be either dict or list.
    """
    if not isinstance(block, (dict, list)):
        return YAMLppError("Block is not dict or list")
    try:
        return block[index]
    except KeyError:
        raise YAMLppError(block, Error.KEY, err_msg)
    except IndexError:
        raise YAMLppError(block, Error.KEY, err_msg)
...
class Interpreter:
    ...
    def __init__(self, filename:str=None, source_dir:str=None):
        ...
        self._jinja2_env = env = Environment(undefined=StrictUndefined)
    ...
    @property
    def stack(self):
        "The contextual Jinja2 stack containing the values"
        # return self._stack
        return self.jinja2_env.globals
    ...
    def evaluate_expression(self, expr: str|Any) -> Node:
        ...
        if not isinstance(expr, str):
            expr = repr(expr)
        template = self.jinja2_env.from_string(expr)
        ...
        if isinstance(node, str):
            ...
            for key, value in node.items():
                ...
                if key == ".context":
                    ...
                    r = self.handle_do(value)
                elif key == ".foreach":
                    r = self.handle_foreach(value)
                    # print("Returned foreach:",)
                elif key == ".switch":
                    r = self.handle_switch(value)
                elif key == ".if":
                    r = self.handle_if(value)
                elif key == ".import":
                    r = self.handle_import(value)
                elif key == ".module":
                    r = self.handle_module(value)
                elif key == ".function":
    ...
    def handle_do(self, sequence: ListNode) -> ListNode:
        """
        ...
        for item in sequence:
            results.append(self.process_node(item))
        return results

    def handle_foreach(self, block: Dict[str, Any]) -> List[Any]:
        """
        Loop
        ...
        """
        # print("\nFOREACH")
        var_name, iterable_expr = get(block, ".values")
    ...
    def handle_switch(self, block: Dict[str, Any]) -> Optional[Any]:
        """
        block = {
            ".expr": "...",
            ".cases": { ... },
        ...
        """
        expr_value = self.evaluate_expression(block[".expr"])
        cases: Dict[Any, Any] = block.get(".cases", {})
        if expr_value in cases:
            return self.process_node(cases[expr_value])
        elif ".default" in block:
            return self.process_node(block[".default"])
        return None


    def handle_if(self, block: Dict[str, Any]) -> Any:
        """
        ...
        """
        r = self.evaluate_expression(block['.cond'])
        # transform the Jinja2 string into a value that can be evaluated
        # condition = dequote(r)
        condition = r
        if condition:
            r = self.process_node(block['.then'])
        else:
            r = self.process_node(block.get(".else", ""))
        # print("handle_if:", r)
        return r



    def handle_import(self, value:str) -> Any:
        """
        Import of an external file
        """
        filename = self.evaluate_expression(value)
        full_filename = os.path.join(self.source_dir, filename)
        _, data = load_yaml(full_filename)
        return self.process_node(data)
    
    def handle_module(self, value:str) -> None:
        """
        ...
        self.jinja2_env.globals.update(variables)
        self.jinja2_env.filters.update(filters)
        return None

    
    def handle_function(self, block:BlockNode):
        """
        Create a function
        ...
        """
        name = get(block,'.name', "Function does not have a name")
        print("Function created with its name!", name)
        self.stack[name] = block
        return None

        
        

    def handle_call(self, block:BlockNode):
        """
        ...
        try:
            function = self.stack[name]
        except KeyError:
            raise YAMLppError(block, Error.KEY, f"Function '{name}' not found!")
        # assign the arguments
        formal_args = get(function, '.args', "Call without arguments")
        args = get(block,'.args')
        if len(args) != len(formal_args):
            raise YAMLppError(block, 
                              Error.ARGUMENTS,
